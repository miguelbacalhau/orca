...
| `githooks/pre-push` | Version-bump guard: a push whose range changes shipped files (`skills/`, `agents/`, `scripts/`, `.claude-plugin/`, `.mcp.json`) without touching the manifest version gets an automatic patch bump committed and the push aborted — run `git push` again to include it. The plugin updater keys its install cache on that version, so an unbumped push makes updates silently serve stale code. One-time setup per clone: `git config core.hooksPath githooks`. |
