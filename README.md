...
/orca:feature
   ...
   │       ──► [one revise round on Critical/High findings, then a re-review]
...
| `scripts/spec.workflow.js` | The gated spec stage: `orca:spec` authors the spec, the run's reviewer adversarially reviews it against the brief and a clean checkout, Critical/High findings drive one revise round and a re-review, and reviewer failures fail open. Started as a one-agent workflow and keeps that family's `{model, effort}` override surface for the spec agent. |
