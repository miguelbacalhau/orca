...
**Escalation** applies one rule: **amend and continue** when a fix preserves the brief's outcome, features, and non-goals (the change touches only *how*) — recorded in the spec's `## Decisions` log with the doubt rule cited; **block and route around** when every fix would change *what* was agreed. Beneath both sits **replan alone**: when the contract is already right and a plan simply got it wrong, the plan is regenerated and `spec.md` is untouched — the replanning agent is told so explicitly, since a note promising an amendment that was never made sends it hunting for a Decisions bullet that does not exist. Whether the spec moved is read from the file (a content hash across the escalation call), never from the escalating agent's self-report. A blocked item keeps its branch — whatever was in its worktree is salvaged into a WIP commit and the worktree removed — for a `/orca:retry` round, and never stalls unaffected work. Under `prefer-smaller-scope`, cleanly cutting a feature counts as an amendment.
