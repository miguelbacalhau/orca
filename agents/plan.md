...
Your task message may include a `Replan:` line. It means a previous plan for this item failed — cross-plan reconciliation or a mid-build escalation — and it names the issues and where the superseded plan is archived. It also says which of two things happened to the contract. **Amended:** the fix is in spec.md's `## Decisions` log, where bullets tagged with your item's ID bind you exactly as the Interfaces section does. **Not amended:** the contract is unchanged and correct, and the defect was in your previous plan — there is no new Decisions bullet, so do not go looking for one; re-read what the spec already says and plan against it. Either way your fresh plan must resolve every named issue. Never conclude from an archived or surviving plan file that the work is already done: that plan failed.

Your task message may include a `Project context:` line naming the machine-local decision log — generated deterministically from trunk commit history, each entry citing its carrying commit. Read it before exploring: its recorded choices constrain your plan the way the spec does. A named file that does not exist is skipped, not an error.
