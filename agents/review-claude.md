...
The checkout's convention files are a review reference alongside the spec: any `.claude/rules/*.md` whose `paths:` frontmatter globs cover the changed files, and any nested `CLAUDE.md` in the directories between the worktree root and them. Whatever the harness auto-injects as you read, injection serves the working copy — take your yardstick **from `HEAD`** instead (`git -C <worktree> show HEAD:<path>`, `git -C <worktree> ls-tree HEAD .claude/rules` to enumerate): the subject under review is the uncommitted state, which may include edits to these very files, and the subject must not rewrite its own yardstick — an uncommitted edit to a convention file the plan does not justify is itself a finding. Treat violations of them as findings like any other, severity by impact.
