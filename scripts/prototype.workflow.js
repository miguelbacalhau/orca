// orca prototype step — the one-agent workflow that spawns orca:prototype.
//
// The prototype skill composes the complete task message (repository root,
// worktree path, branch, run dir, the micro-brief verbatim, optional Project
...
// prototype.effort an ordinary override: the Agent tool has a model
// parameter but no effort parameter, while agent() here takes both alongside
// agentType — so the prototype step gets the same {model, effort} tuning
// surface as every workflow-spawned stage.
//
// args: {
//   prompt   the complete task message for the orca:prototype agent,
//            composed by the skill. This script never reads files — the
//            workflow sandbox has no filesystem access
//   model    optional model override (agents.prototype.model from the config)
//   effort   optional effort override (agents.prototype.effort from the config)
// }
//
// Return: { summary: <the agent's final text>, died: false } — or
//         { summary: null, died: true } when the agent was skipped or died
//         on a terminal API error: a typed failure the skill branches on
//         (relaunch once, then report and stop), never a shapeless null. No
//         schema on the agent call: the prototype agent's contract is prose
//         — its final message is the report material the skill writes to
//         report.md.
//
// No resume plumbing, deliberately: this is one agent call — cheap to redo.
// An interrupted prototype is abandoned or relaunched fresh; the runId is
// throwaway and is never persisted anywhere.

export const meta = {
  name: 'orca-prototype',
  description: 'Hack the idea into a throwaway spike — one orca:prototype agent',
  phases: [{ title: 'Build' }],
}
...
// debug-loop.workflow.js, spec.workflow.js, research.workflow.js, and this
// script: a value accepted by any validator but rejected by another bricks
// that verb's launches until the config file is hand-edited. Workflow
...
const opts = { agentType: 'orca:prototype', label: 'prototype', phase: 'Build' }
