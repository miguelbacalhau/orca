...
- **`agents`** — per-stage `{model, effort}` overrides. One block serves both verbs: the feature stages (`spec` through `integrate`) and the debug stages (`reproduce`, `hypothesize`, `verify`, `diagnose`) live side by side, and each run applies its own verb's keys — plus, for a debug run's nested fix tail, the feature stages except `spec` (the fix contract is written by the diagnose agent, so a `spec` override never applies to debug runs) — ignoring the rest. The block also carries `research`, the feature interview's exploration agent: it belongs to no run — the interview reads it fresh at each research spawn, before any run exists, and both runs validate-and-ignore it.
...
- This skill configures the reviewer choice, the orca:review `editor`/`terminal` keys, and models/effort only. The codex machine setup (binary, auth, review timeout) and the orca.nvim / orca.vscode installs belong to orca:doctor; repository layout belongs to orca:init; run behavior belongs to orca:feature and orca:debug; opening the review itself belongs to orca:review.
