...
**The orca status line check.** Per-user (offered in machine-only mode too — settings files need no repository). Read the `statusLine` key in `~/.claude/settings.json` and, inside a repository, the project's `.claude/settings.local.json`, then report which of three states the session is in: **installed** (a command invoking `orca.sh statusline`), **absent** (no `statusLine` configured anywhere), or **foreign** (a `statusLine` that is not orca's). For installed, verify the absolute path the command names still exists — a plugin update can move the install root and leave a dead command; the fix is rewriting the block with the current `${CLAUDE_PLUGIN_ROOT}`, consented like every settings write. For absent, offer the block orca:init offers — `"statusLine": { "type": "command", "command": "bash <plugin-root>/scripts/orca.sh statusline", "refreshInterval": 5 }`, path substituted (`refreshInterval` is what keeps the board moving while the main session sits blocked on the Workflow tool) — merged into the file of the user's choice. For foreign, never touch or merge into it: print the composable one-liner `bash <plugin-root>/scripts/orca.sh statusline` for the user to chain into their own script — it prints nothing when no run is active. Every write shares the session-start caveat: a fresh session before it takes effect.
