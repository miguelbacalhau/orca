---
description: Drive a feature from idea to committed code using a deterministic workflow over dedicated subagents — an autonomous orca feature run. A dispatcher over `.orca` state: invoked with an idea argument, the argument is the intent and triage goes straight to the interview seeded with it; without one, triage offers to resume an interrupted run, to run a queued brief from `.orca/feat-briefs/`, or — with nothing waiting — interviews the idea into a durable brief (researching the touched subsystems through a subagent first, so the questions come from real friction with the code), then offers to run it now or leave it queued. The run restates the brief and confirms it once, writes a spec with a dependency-ordered work breakdown — independently reviewed against the brief and the codebase before launch, with one bounded revise round — then executes the work loop as a single Workflow-tool run that, per unblocked item, plans and implements in its own git worktree branched off a shared bare repository, reviews with an independent reviewer — cross-model Codex where it is installed or pinned, a dedicated fresh-context Claude reviewer otherwise — plus a Claude fix agent, commits, and serially merges into an integration worktree. Requires a bare-repo-with-worktrees layout (validated up front) and a harness with the Workflow tool; there is no privileged main checkout, and the run's deliverable is a branch the user lands themselves. Triage may present its closed-set choices as a picker; after the opening confirmation — plus one optional breakdown checkpoint the brief opts into — the run asks the user nothing else, and undecidable issues are reported at the end. Do not use for small single-file changes or when the user only wants a spec or a plan.
args: <idea>
user-invocable: true
...
The brief is where the user sets intent — written in this skill's interview or queued by an earlier one, discovered on disk, and confirmed once at the start of the run. It may also opt into a single checkpoint: a one-time review of the spec and work breakdown before any code is written. From the opening confirmation on — apart from that opt-in checkpoint — never ask the user anything: no mid-run clarifications, no approval gates, no AskUserQuestion. (Step 0's triage runs *before* the confirmation and sits outside this rule; its picker is the one sanctioned use of AskUserQuestion.) The workflow runs autonomously in the background and could not pause for a question even if one arose; anything it cannot decide within the brief's stated intent becomes a `blocked` item surfaced in the final report.
...
**The argument gate — an argument is the intent.** Invoked with an `<idea>` argument, skip checks 2 and 3 and go straight to the interview (check 3's `interview.md`), seeded with it. Do not offer queued briefs, do not ask which to run, no picker — the user just typed the answer. Run the triage call below anyway (it is one cheap read), because two kinds of state still surface as **one-line mentions that never block**:

- An interrupted run: *(note: `20260806-1512-feat-status` is still unfinished — `/orca:feature` with no argument will offer to resume it)* — then continue.
- A queued brief that looks related: *"a queued brief looks related: `auth.md`"* — and the user may ask for the fold; never fold silently. (Folding an argument into a matching brief used to be automatic; deciding sameness is a judgment call, and the argument path sheds judgment calls.)

One thing blocks, precisely: a `LEASE: live` on a run whose slug/scope **overlaps the argument**. The only hard conflict a lease guards is two writers on the *same* run directory, and an overlapping argument is heading there; a live lease on an *unrelated* run never blocks — separate run dirs, separate branches, separate worktrees — it is just a prominent mention like the others. The lease state is a fact; the overlap is your judgment against the slug and the argument text, leaning literal (slug tokens appearing in the argument), never a fuzzy theme match — when in doubt, mention rather than block. And `live` means *owned by an open session*, not *executing right now* — phrase mentions accordingly.

**Offering choices — the picker.** A triage pick is a closed set of on-disk states, so it is a legitimate AskUserQuestion (single-select; the interview's ban on the tool starts only after the Step 1 confirmation). Decode each candidate's `ACTION:` evidence — and, for briefs, the file's opening lines — into the option's `preview`. Rules:

- **1 candidate → no picker.** A prose offer; a one-item menu is more friction than a sentence (and the tool requires two options anyway).
- **2–4 candidates → picker**, always including "interview something new" as an explicit option — the never-force rule made structural.
- **>4 → full list as text first**, then a picker of the top three plus "interview something new" (exactly the four-option cap; "Other" is added automatically). Silent truncation to four would hide a real state.
- **With an argument → no picker at all.** The gate above already answered.
...
Each `RUN:` line is a feature run directory with no `report.md` yet, tagged `interrupted` — its workflow launched, and the `RUNID:`/`ARGS:` lines that follow carry the resume handle persisted in its `spec.md`, extracted byte-exact (plus legacy `REVIEWER:`/`AGENTS:` lines for runs recorded before the args line existed) — or `unlaunched` — it died before its workflow launched. An unlaunched dir holding only `brief.md` (no `spec.md`) died between consuming its brief and writing the spec: recover it by offering the run exactly like a queued brief, reusing the same run directory and its on-disk brief — no new run dir, no `mv`, continue from the Step 1 confirmation. An unlaunched dir WITH a `spec.md` is not resumable; leave it alone. The same call emits check 2's `BRIEF:` lines, plus `CASE:` lines that belong to orca:debug's triage, `DONE:` lines (finished runs, each tagged `clean`, `leftovers`, or `unknown`) that belong to the recovery skills — when one shows `leftovers` (or `unknown`), mention that `/orca:retry` finishes its unmet items; when `clean`, that `/orca:followup` handles any follow-ups; a mention only, this skill dispatches on `RUN:` and `BRIEF:` lines alone — and git-footprint and `ACTION:` lines that belong to orca:status's dashboard. The `ACTION:` lines are the script's own ranked routing: act on the ones tagged `feature` (`resume-run`, `run-brief`) as the candidate list for the offers below — render them, never re-derive them. Empty output means nothing is waiting; nothing is read into context.

Each `RUN:` line is followed by a `LEASE:` verdict — the liveness question answered on-disk, no longer guessed:

- `live` — **an open session on this host owns this run.** Not necessarily executing: the pid is the owning session's process, so a session whose workflow was stopped, or that was `/clear`ed, reads `live` too. If it is *this* session's own workflow (check the task list), say the run is in flight; otherwise present it as owned by an open session. Either way it is not a resume candidate (the script emits no `resume-run` action for it) — mention it and fall through to the next check.
- `stale` — the holder provably crashed (pid gone, or its start time changed). Offer the resume; the claim below recovers the lock without asking.
- `none` — no lock at all (e.g. the session died before the workflow launched, or after it released).
- `unknown` — another machine, or an unreadable owner file: the one case where liveness is genuinely unknowable from here. Offer the resume, but say another host may be driving the run — the user would know.

Offer to resume — one candidate as prose, several through the picker rules above. If the user declines in favor of new work, fall through to the next check.

To resume — never by re-running stages conversationally: take the `RUNID:` and `ARGS:` values the discovery emitted (the `**Workflow run:**`/`**Workflow args:**` lines persisted at the end of `spec.md`, extracted byte-exact), then re-invoke the Workflow tool exactly as Step 4 specifies — same `scriptPath`, the emitted args **verbatim**, plus `resumeFromRunId: "<RUNID>"`. **Claim the run first, at action time:** a resume replays the workflow's journaled claim without re-executing it, which also means no owner-file refresh — left alone, a live resumed run would still carry the dead session's identity and read `stale`. So immediately before invoking the Workflow tool, take the lease as this session:

```bash
bash ${CLAUDE_PLUGIN_ROOT}/scripts/orca.sh triage claim --steal --runid <RUNID> <run-dir> 'orca feature resume; slug=<slug>'
```

`--steal` recovers a provably-`stale` lock atomically (rename-first — no contender can remove a fresh lock) and refuses anything else typed: a `NOT_STALE`/`LEASE_HELD` refusal means an open session owns the run *right now* — stop and tell the user; the triage snapshot was minutes old, and the claim's own check at launch time is the one that counts. On success, **hold the lease through the launch**: the workflow's journaled claim replays as a no-op, and its release at the end runs live and cleans up.

Verbatim matters: any drift — item order, title wording, a `files` array — changes agent prompts and makes completed stages re-run instead of replaying from the journal. An `ARGS:` of `absent` marks a run recorded before the args line existed — fall back to rebuilding `args` from the spec's Work Breakdown, the reviewer from the discovery's `REVIEWER:` line (`absent` → `codex`, the only reviewer that existed when reviewer-less specs were written), and the agents block from its `AGENTS:` line (`absent` → omit the key) — never from `.orca/config`, which may have changed since launch. Completed agent calls replay instantly from the journal; only in-flight and remaining work runs live. Stage prompt text is part of the journal key too: a run launched under a plugin version whose stage prompts carried status wording (`Status task:` lines with per-item `taskId`s in the args, or the later `Status file:` lines) re-runs its in-flight items' stages on resume. Pass its recorded args verbatim anyway, `taskId`s included: the workflow ignores unknown item fields, so correctness is preserved and only replay economy is lost. Before resuming, reconcile leftover git state (`git worktree list`) only if the journal and the worktrees disagree. Step 1's permissions pre-flight applies to a resume too — live stages will spawn agents. Its graceful decline applies with one correction: on a resume there is no queued brief to report — the brief was consumed into the run directory at launch — so report instead that the run stays resumable from its journal, and that enabling bypass (Shift+Tab) and re-invoking `/orca:feature` will rediscover it in triage. When the resumed workflow completes, continue at Step 5.
...
- **Briefs waiting:** present the filenames — one as a prose offer, several through the Step 0 picker rules (previews from each brief's opening lines) — and offer to run one, or to interview a new idea instead. Read only the chosen brief and proceed to Step 1 with it.

(An `<idea>` argument never reaches this check — the argument gate at the top of Step 0 goes straight to the interview, mentioning any related brief on the way; folding happens only when the user asks for it.)
...
- `CODEX: PASS|FAIL|SKIPPED` — checked only when the resolved reviewer is codex: the global `codex` binary is on PATH at or above the minimum version, authenticated, still carrying the `codex exec` flags the transport drives, and `BASH_MAX_TIMEOUT_MS` set in a settings env block. With reviewer claude it prints `SKIPPED` — not checked, deliberately.
- `RESULT: PASS|FAIL` — mirrored by the exit code, which is 0 iff every gate passed.

There is no agents gate: the stage agents ship inside this plugin, so a session that can run this skill has them by construction.
...
There is no second, session-only gate. The codex reviewer used to be reached over MCP, which meant a registration that had to be loaded at session start and could not be checked from a script; it is now reached by running the binary through `orca.sh codex`, so every codex question is a shell question and the pre-flight above answers all of them.
...
- **Codex**, when it is the reviewer: a missing, stale, unauthenticated, or moved reviewer would fail every item at review time, deep into an autonomous run. The reviewer is the **global codex binary**, run non-interactively by `orca.sh codex` (`codex exec`) — codex is **never installed via npm**: the only supported install is the system one on PATH, from the official non-npm distribution (e.g. `brew install codex`, or the release binaries). The `codex exec` flag check is not redundant with the version check: the CLI removed the `codex mcp-server` subcommand orca used through 0.32.0 without deprecating it, and a version-and-auth gate passed happily while every review failed — so the gate now asks the binary what it speaks. Whatever the failing check — install, upgrade, `codex login`, a moved interface, or the `BASH_MAX_TIMEOUT_MS` settings write — point the user at the **orca:doctor skill**, which walks the machine gates interactively (a plugin cannot set session env, so that knob stays a per-repo or per-user settings write). Installing, authenticating, and settings writes are the user's actions — surface the fix, do not do it autonomously.
...
└── .orca/YYYYMMDD-HHMMSS-feat-<slug>/
    ...
    ├── merged.tsv  # <ID>	<sha> per landed item — audit's join, written by the merge backstop
...
### Project context: render the decision log

The run's judgment stages start from one machine-local file at the top level of `.orca/`: `decisions.md`, the decision log — rendered deterministically from trunk commit history, never committed, never hand-maintained. Before spawning the spec agent, render it current:

```bash
bash ${CLAUDE_PLUGIN_ROOT}/scripts/orca.sh decisions render <repo-root> --trunk <trunk>
```

The verb extracts every canonical `chose X over Y: <reason>` bullet from the commit bodies reachable from the trunk tip (sha-derived `D-<short-sha>` entry ids, newest first) and appends a `decisions.archive.md` verbatim when one exists — the frozen pre-programmatic log. Trunk-only by design: decisions on an unmerged deliverable branch stay invisible until the human's merge lands them, so decision visibility always matches code visibility.

Failure here is non-fatal: a run with a stale or missing decision log still runs — every consuming agent treats a missing file as skippable.
...
- a `Project context:` line naming `<repo-root>/.orca/decisions.md` (decision log), framed as generated from trunk commit history with each entry citing its carrying commit — recorded choices are authoritative unless the brief deliberately overrides one; a missing file is skipped, not an error
...
**Create the spec-review worktree.** The spec the agent writes gets an independent adversarial review — by the run's held reviewer, against the brief and the codebase — inside the same workflow, before anything launches over it. The reviewer works in a clean throwaway checkout, never the user's worktree: both reviewer variants see identical inputs, with no uncommitted state and no placed secrets. Create it immediately before the workflow invocation, at the tip of the trunk confirmed in Step 1:

```bash
git worktree add --detach <repo-root>/orca-<slug>-specreview <trunk-tip>
```

No branch, no `secrets place` — clean by construction. Every spec-workflow invocation (a Step 3 checkpoint re-spawn included) is bracketed by this create and the matching remove below.

**Spawn the spec agent through its bundled workflow.** Invoke the Workflow tool with `scriptPath: "${CLAUDE_PLUGIN_ROOT}/scripts/spec.workflow.js"` — the substituted value is already absolute; never pass `~` or an unsubstituted variable (same rule as Step 4) — and `args: { prompt, model?, effort?, runDir, reviewWorktree, reviewer, pluginRoot }`: `prompt` is the composed task message; `model`/`effort` come from the held `agents.spec` block, each passed only when set (both apply on every spec spawn — the workflow's own revise round and a Step 3 checkpoint re-spawn included); `runDir` and `reviewWorktree` are the absolute run-directory and review-worktree paths; `reviewer` is the held reviewer from this step — the spec reviewer is the run's reviewer, no separate knob; `pluginRoot` is `${CLAUDE_PLUGIN_ROOT}` substituted (the codex reviewer shells out to `orca.sh codex` and nothing in a subagent's environment carries that path). The workflow runs the gated sequence — spec, independent spec review (`orca:spec-review-<reviewer>`, writing `<run-dir>/reviews/spec-<reviewer>.json`), and on Critical/High findings exactly one revise round that folds them into the spec, with no re-review — its output is the spec that launches — in the background: wait for its task notification before proceeding — do not poll `spec.md`, and never fabricate the result; its `log()` lines are not visible mid-run ([anthropics/claude-code#74419](https://github.com/anthropics/claude-code/issues/74419)).

It returns `{ summary, died, review }`. **Remove the review worktree first** — `git worktree remove <repo-root>/orca-<slug>-specreview` — on every path out of this step, the failure and stop paths included. Then branch on the result:

- `died: true` (`review` absent): relaunch the workflow once, re-creating the review worktree first; if the second launch dies too, report the failure to the user and stop the run before anything else is spent.
- `review.written` with `criticalHigh > 0` and `revised: true`: the revise round addressed (or explicitly rebutted, as Risks entries) the findings, and its output is the spec that launches — proceed, carrying the counts into the Step 3 announce. There is deliberately no re-review: a second adversarial pass is blind to the first and churns rather than converges, and the item reviews and integration verification still gate everything built over the spec.
- `review.written` with `criticalHigh > 0` and `revised: false`: the revise spawn died and the Critical/High findings stand unaddressed. This is the same posture as a spec agent reporting an unsplittable scope: surface the findings to the user, pointing at `<run-dir>/reviews/spec-<reviewer>.json` as the evidence, and stop rather than launching against a spec known to be wrong — creating no worktrees, branches, or session tasks beyond what already exists.
- `review.written: false`: the review failed open — a reviewer failure must not brick the run. Relay `review.reason` as a one-way status line and proceed; the spec stands unreviewed, and every downstream gate (item reviews, integration verification) still stands.
- otherwise (`criticalHigh: 0`): clean — proceed.

The spec workflow's runId is throwaway — never persist it and never resume it; an interrupted spec stage relaunches fresh.
...
The spec agent authors `spec.md` **once**. During the run, mid-loop amendments — interface revisions, breakdown changes, the `## Decisions` log — are made by the workflow's escalation agent (Step 4), never by a re-spawn; the exceptions are the spec workflow's own bounded revise round (a Critical/High spec review re-spawns the spec agent once, inside the workflow) and a structural revision requested at the Step 3 checkpoint, which re-spawns the spec agent (see Step 3).
...
Report the spec to the user — outcome, work items, dependency order, what will run in parallel, key assumptions — plus one line on the spec review's outcome: the reviewer, the finding counts, and whether a revise round ran (or that the review failed open). One-way status, like the rest of the announce. How this lands depends on the breakdown-checkpoint choice recorded in the brief:
...
Last, claim the run directory — from here to the launch this session is its writer, and unleased, the freshly scaffolded run would read as abandoned to any concurrent triage:

```bash
bash ${CLAUDE_PLUGIN_ROOT}/scripts/orca.sh triage claim <run-dir> 'orca feature skill; slug=<slug>; pre-launch'
```

Step 4 releases it immediately before the workflow takes over.
...
- **Completion-driven scheduling** — an item launches the moment its own dependencies merge and advances the moment its previous stage finishes; it never waits for an unrelated sibling. The only synchronization points are the ones that should exist: the spec-editing escalate section (waves take it one at a time; the reconciliation *read* ahead of it does not, being read-only and hash-guarded) and the serialized merge queue.
- **Bounded loops** — max 2 fix rounds gated on Critical/High findings; the commit-attribution check runs against the actual `git log` message (never the agent's self-report), with two attempts and then a deterministic message rewrite, and each item's squash onto the integration branch gets the same read-back check; one escalation round per reconciliation failure, always re-reconciled after it — advisory on that second pass: an objection that survives rides into the item's implement and review prompts instead of blocking it before any worktree exists; at most one escalation-backed replan-and-rebuild per item after a spec-rooted mid-build failure.
...
**Release the skill's claim immediately before this call** — `bash ${CLAUDE_PLUGIN_ROOT}/scripts/orca.sh triage release <run-dir>` — then invoke: the workflow takes its own lease at launch, through the same claim verb, and that claim is deliberately **not re-entrant** — a same-session relaunch over a live workflow carries the same session pid and must still refuse, which is exactly the interleaved-writers case the lease exists to stop; release-before-launch is the handoff, and the unprotected gap is a single tool call. A resume (`resumeFromRunId`) replays the lease from the journal and is never blocked by the lock this session already holds from Step 0's resume claim. On a typed `LEASE_HELD` refusal, read the verdict it names: `live` — an open session owns this run; stop and report rather than fight it. `stale` — the holder provably crashed; recover with `triage claim --steal <run-dir> <note>` followed by `triage release <run-dir>`, then relaunch. Never hand-remove a `.lock`. Persist the resume handle from the tool result immediately: append a `**Workflow run:** <runId>` line to the end of `spec.md` and a `**Workflow args:** <the args object as one-line JSON, exactly as passed>` line beside it. The args line is what a resume replays **verbatim** — any rebuild drift (item order, title wording, a `files` array) would change agent prompts and silently re-run completed stages instead of replaying them. That interruption — session death — erases the conversation, the only other place the launch-time values exist, and `.orca/config` may have been edited or reset since launch, so the config file can never stand in for these lines.

### What the workflow does

Per readiness wave (the items whose dependencies have all merged at that moment): plan agents in parallel → **when the wave has two or more live plans**, an opus reconciliation agent reads them against each other and against the spec Interfaces (a one-plan wave has no cross-plan subject and takes no gate); its charter is conflicts only — an undeclared dependency between the items, two plans assuming different shapes for one shared contract, heavy file overlap, a plan contradicting a defined Interfaces clause — with plan quality, acceptance-gate wording, and file-ownership bookkeeping explicitly out of scope, since the item review reads the built diff and the merge stage resolves overlap with both plans in hand. The call is read-only, so it runs outside the spec section, bracketed by a `spec.md` content hash: clean over an unmoved spec finishes there; dirty, or a spec that moved mid-read, takes the section (reusing the verdict when the hash held). → On issues, an escalation agent applies the **amend-vs-block** rule (below): it edits `spec.md` itself for amendments, or — when the contract is fine and a plan is not — regenerates that plan and edits nothing. Whether it amended is read back from a `spec.md` hash, not from its self-report, so the replanning agent is told truthfully which happened. The affected plans are regenerated once and the wave is re-reconciled; an objection that survives that second pass no longer blocks the item — it is seeded into the item's implement and review prompts as a pre-existing objection to check, so the round ends with a branch and findings rather than an empty branch and a report. A dependency an amendment adds between items is reported structurally and applied to the scheduler itself — a wave item whose new dependency has not merged yet is deferred and relaunched when it has. Then each ready item pipelines independently: worktree off the integration tip (a worktree left by an interrupted run is resumed in place, and a blocked item's kept branch — salvaged WIP commit included — gets its worktree re-added, never a collision) → `orca:implement` → independent review (with reviewer codex, an `orca:review-codex` agent drives Codex through `orca.sh codex`, read-only in the item's worktree, and the verb lands the findings JSON verbatim; with reviewer claude, an `orca:review-claude` agent performs the review itself under the same adversarial contract and writes findings in the identical schema — either way the artifact plus a per-round archive land in `reviews/`, and a dead reviewer, unparseable payload, or tripped self-check comes back as a failure the workflow retries, never as a clean pass) → `orca:fix` → re-review, gating commit on zero Critical/High (a first review with no Critical/High skips the fix round entirely — Medium/Low findings stay on record in `reviews/` and are not a gate, since the exit never keyed on them) → `orca:commit` with the attribution check → serialized `orca:merge` in the integration worktree, self-healing any half-finished merge state first and squashing the item onto the integration branch under its own commit message, so the branch stays linear and carries no run-internal item id → best-effort worktree cleanup that can never demote a merged item. A mid-build failure that smells spec-rooted — an implementation reported infeasible, fix rounds exhausted on Critical/High findings, a semantically aborted merge — goes to the escalation agent once: an amendment replans and rebuilds the item once in its kept worktree — bracketed by the same `spec.md` hash read, so this replanning agent is told just as truthfully whether the contract actually moved; otherwise the item is cut (under prefer-smaller-scope) or blocked. After the loop drains, `orca:integrate` verifies the assembled feature; if it applied fixes, they pass the same bounded review → fix → re-review gate as items (zero Critical/High within two rounds) and are committed (attribution-checked) only on a clean pass — a failed or blocked review leaves them uncommitted in the integration worktree, records the gap, and marks the run's deliverable state `unverified` rather than shipping the one unreviewed edit of the run. There is no post-run context stage: the run's decisions travel in its commit bodies, and the machine-local decision log is re-rendered from trunk history at the next run's launch — nothing becomes globally visible before the human merges the branch.

The escalation rule the workflow's agent applies is unchanged from the conversational design: **amend and continue** when a fix preserves the brief's outcome, features, and non-goals — the change touches only *how* — recording the decision in the spec's `## Decisions` log with the doubt rule cited; **block and route around** when every fix would change *what* was agreed, recording the reason and the options the user must choose between. One outcome sits below both: **replan alone**, when the contract is already right and a plan simply got it wrong — the plan is regenerated and `spec.md` is not touched, and an amendment invented to justify a replan would rewrite the contract every later item is bound by. A blocked item keeps its **branch** — whatever was in its worktree is salvaged into a WIP commit and the worktree removed — for a `/orca:retry` round, and never stalls unaffected work. When the doubt rule is prefer-smaller-scope and a feature can be cleanly cut rather than blocked, cutting it is an amendment.

### While it runs and after

The workflow runs in the background, but its `log()` lines (items merged, items blocked, amendments) are **not visible on any live surface while the run is in flight** ([anthropics/claude-code#74419](https://github.com/anthropics/claude-code/issues/74419)) — they land only in the `logs` array of the completion-time `workflows/<runId>.json`, where notable ones are read back for the report; mid-run, the only live surface is the Workflow tool's own progress display. When it completes, it returns `{ shipped, cut, blocked, integration, deliverableState, objections, tokensSpent }` — `deliverableState` is the run's terminal verdict on the branch: `verified` (the integrate agent completed and the branch tip is exactly the tree it verified), `unverified` (merged work exists but the verifier died or its fixes never landed reviewed — never describe such a branch as a completed deliverable), or `built` (nothing merged, verification never ran); `objections` is `{ id, issues }` per item whose plan carried a reconciliation objection the gate could not resolve:

- Blocked items' branches were kept, each holding a salvaged WIP commit when partial work existed — list them for `/orca:retry`.
- Items named in `objections` carried a plan objection the gate could not resolve. One that shipped built over it rather than being blocked before any code existed — its review saw the objection and judged it against the real diff, so it is not automatically a defect, but the report must say the item shipped carrying it. One that also appears in `blocked` or `cut` never built; there the objection is context for `/orca:retry`, not a shipped risk.
- Proceed to Step 5; the returned values feed the report directly.
...
- <From the returned `objections`: each *shipped* item that built over a plan objection the reconciliation gate never resolved, with the objection's own words. Say that the item's review checked it against the built diff and did not block on it, so this is a place to look rather than a known defect. An item that is also in `blocked` or `cut` belongs in its own section, with the objection as context there. Omit the line entirely when `objections` is empty.>

## Knowledge worth promoting

- <Rule-shaped knowledge or documentation the run surfaced (from the spec's Decisions and the plans' Deviations) that belongs in CLAUDE.md or the repo's real docs. Orca proposes; the human commits it under their own name — no orca agent ever writes CLAUDE.md. "None" if empty.>
...
- Each stage is a dedicated subagent type (`orca:spec`, `orca:plan`, `orca:implement`, `orca:review-codex` or `orca:review-claude` per the run's reviewer, `orca:fix`, `orca:commit`, `orca:merge`, `orca:integrate`) whose instructions are its definition, loaded as its system prompt. The workflow spawns them by `agentType` and passes only the per-item values — run directory, worktree path, ID and title, owned files, branch names, artifact paths; the spec agent is spawned in Step 2 through its own gated workflow, with the brief and repo root in its prompt, followed there by an independent spec review (`orca:spec-review-codex` or `orca:spec-review-claude`, per the same reviewer). The heavy stage instructions never enter the main context. Review runs through the same shape either way: with reviewer codex, the `orca:review-codex` agent carries the static adversarial template and hands it to `orca.sh codex`, which runs the binary read-only with the item's worktree as cwd and writes the findings JSON to both paths itself — verbatim by construction, since no agent transcribes it; with reviewer claude, the `orca:review-claude` agent performs the review itself under the same adversarial contract and writes the artifact. Both land `<run-dir>/reviews/<ID>-<reviewer>.json` plus a per-round archive in the identical schema and return the counts the merge gate branches on. The counts are the review agent's own count of the findings on disk; the CLI validates the complete JSON document before publishing it, without counting or interpreting findings.
