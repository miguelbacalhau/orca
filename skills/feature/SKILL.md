...
- **Bounded loops** — max 2 fix rounds gated on Critical/High findings; the commit-attribution check runs against the actual `git log` message (never the agent's self-report), with two attempts and then a deterministic message rewrite, and each item's squash onto the integration branch gets the same read-back check; one amendment round per reconciliation failure, always re-reconciled before building; at most one escalation-backed replan-and-rebuild per item after a spec-rooted mid-build failure.
...
**Seed the status files.** Once the breakdown is final, create `<run-dir>/status/` and write the single word `pending` into one file per work item — `<run-dir>/status/Wn` — the live per-item surface during the run: the script threads a `Status file:` line into every stage prompt, and the stage agents overwrite their own item's word as they run (`planning`, `implementing`, `review`, `fixing`, `committing`, `merging`; only the merge stage writes `merged`), while the orca status line renders the board (installed via orca:init; `orca:doctor` reports its state). A dependency-blocked item needs no writes from anyone — it truthfully sits `pending`, and the renderer names its unmerged blockers from the args' `deps`. This layer is display-only and fail-soft: if seeding fails, launch anyway — the renderer treats a missing file as `pending`. The `integration` pseudo-item never gets a status file.
...
The workflow runs in the background, but its `log()` lines (items merged, items blocked, amendments) are **not visible on any live surface while the run is in flight** ([anthropics/claude-code#74419](https://github.com/anthropics/claude-code/issues/74419)) — they land only in the `logs` array of the completion-time `workflows/<runId>.json`, where notable ones are read back for the report. The live mid-run surface is the **status files** seeded above: the script threads a `Status file:` line into every stage prompt, and each stage agent's first action writes its stage word to `<run-dir>/status/Wn`, which the orca status line (installed via orca:init, rendered by `orca.sh statusline` on a refresh interval) turns into a per-item board. Dependency-blocked items sit `pending` with their unmerged blockers named by the renderer, and only the merge stage writes the terminal `merged` — every other stage's line forbids it, so a file reading `merged` early is a stage agent violating its line, not the design. When it completes, it returns `{ shipped, cut, blocked, integration, deliverableState, promotions, tokensSpent }` — `deliverableState` is the run's terminal verdict on the branch: `verified` (the integrate agent completed and the branch tip is exactly the tree it verified), `unverified` (merged work exists but the verifier died or its fixes never landed reviewed — never describe such a branch as a completed deliverable), or `built` (nothing merged, verification never ran):
...
Reconcile the status files first, from the returned values — the stage agents' writes are best-effort, so the terminal words are set here, in `<run-dir>/status/`: every `shipped` item's file → `merged` (a backstop for a merge agent whose final write was skipped), every `cut` item's file → `cut`, every `blocked` item's file → `blocked — <short reason>` (the one word allowed a suffix; renderers key on the first word). Fail-soft as ever: a failed write never stops the report.
