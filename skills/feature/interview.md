...
Compose the research task message: the repository root, the idea verbatim, and — when `<repo-root>/.orca/map.md` or `.orca/decisions.md` exists — a `Project context:` line naming whichever of the two exist. Only check that the files exist; do not read them in the main context — the agent scopes its own exploration from the idea plus the map, and its definition owns the report's shape: current behavior, decisions touched, tensions, unknowns.
