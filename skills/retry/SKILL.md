...
Spawn **one `orca:audit` subagent**, passing the repository root (the parent of `git rev-parse --path-format=absolute --git-common-dir`), the run directory, and that **orca:retry** is asking. The agent reads the run's artifacts, verifies the report's claims against the work breakdown and git, and returns a compact reconciliation as its final message. All heavy reading lives and dies there — the main conversation consumes only the returned report. Retry consumes the **completion picture** (which items verifiably merged — the run's `merged.tsv` ledger confirmed against git, not the Shipped table), the **discrepancies**, the **unfinished items** (with their blocked reasons and surviving branches), and the **escalated decisions**.
