...
- **Fresh status files** for the retried items only: seed `<run-dir>/status/<id>` with the single word `pending` per retried item, overwriting the prior round's `blocked — <reason>` (create `status/` first if a run from before the status files lacks it); untouched items' terminal words stay in place.
