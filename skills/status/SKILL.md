...
- **In flight / owned** — a `RUN:` whose lease is `live` gets no resume action, deliberately: an open session owns it. Render it first, as *owned by an open session (not necessarily executing — the owner may be idle)*; never present it as resumable, and never claim it is dead.
- **Interrupted run** (`resume-run`) → resumable via `/orca:feature` — the lease said `stale`, `none`, or `unknown`; on `unknown` (another host, or an unreadable owner file) add that another machine may be driving it — only the user knows.
...
- **Archived** (`ARCHIVED:` lines) — runs the user retired through `/orca:archive`: landed, clean, and out of the routing. They carry no action and no enrichment by construction. Render them as a **one-line count** at the very bottom ("3 archived runs"), never itemized — the whole point of the marker is that finished history stops competing for attention. Name a directory only when the argument filter matched one.

**Offer the retirement when the list has earned it.** Finished runs are never removed on their own, so a long-lived repository's triage grows with its history: every entry-point skill re-reads it, and after a dozen runs the finished ones dominate a screen meant to show what is waiting. When the "finished clean, landed" group holds **three or more** runs, close the screen with one line pointing at `/orca:archive`, which retires the provably-landed ones behind a consent gate — and removes their merged branches and clean worktrees with them, so it is also the one-step answer to a long "safe to delete" list. A pointer, not a nag: say it once, never repeat it per run, and never imply anything is lost — `/orca:followup` still picks archived runs, their reports keep every deferred item, and `/orca:archive --undo` reverses the marker and recreates the branches.
...
- **No mutations.** Not even "obviously safe" ones: no `git worktree remove`, no `git branch -D`, no re-queuing a brief, no removing a stale lock, no archiving a landed run, no consent-per-step cleanup mode. The triage verb's read-only boundary runs per-subcommand — `snapshot` (this skill's one call) is strictly read-only, while `claim`/`release` and `archive`/`unarchive` mutate — and status never touches a mutating pair; it points at `/orca:archive` and stops. A dashboard users run reflexively must have no hands — one misread and a mutating status kills a resumable run. Prescribe; never execute.
...
- **No readiness.** Whether the machine and the repository are *set up* — codex, the review timeout, `.orca/config`, and whether `.orca/setup` provisions fresh worktrees — is `/orca:doctor`'s, by the test that its answer would be the same if no run had ever happened. Status answers the complement: what a run left behind. A report or an item blocked on an unprovisioned worktree still renders here as run state, with `/orca:doctor` named as where the static cause gets fixed.
